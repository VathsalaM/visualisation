var points = [
    {x: 0, y: 5}, {x: 1, y: 9}, {x: 2, y: 7}, {x: 3, y: 5}, {x: 4, y: 3}, {x: 6, y: 4}, {x: 7, y: 2}, {
        x: 8,
        y: 3
    }, {x: 9, y: 2}
];
...
var addCircle = function (group, data, yFunc) {
    group.append('g').selectAll('circle').data(data).enter().append('circle')
        .attr('cx', function (p) {
            return xScale(decimal(p.x))
        })
        .attr('cy', function (p) {
            return yScale(yFunc(p.y))
        })
        .classed('scale', true)
...
var createLine = function (curve, xFunc, yFunc, xKey, yKey) {
    return d3.line()
        .curve(curve)
        .x(function (p) {
            return xScale(xFunc(p[xKey]))
        })
        .y(function (p) {
            return yScale(yFunc(p[yKey]))
        });
};
...
var renderCharts = function () {
    ...
    var ex12data = [generatePoints(function (number) {
        return ex12Equation(number)
    }, 0, 10)];
